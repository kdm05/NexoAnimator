...
5local tweeninfo = TweenInfo.new
...
local themes = {
	...
	Accent = Color3.fromRGB(0, 40, 185), 
...
do
	...
	function section:addSlider(title, default, min, max, callback)
      ...
      ["."] = true,
		...
		textbox.FocusLost:Connect(function()
			if not tostring(textbox.Text) then
		...
		textbox:GetPropertyChangedSignal("Text"):Connect(function()
			...
			if not allowed[text] and not tostring(text) then
				textbox.Text = text:sub(1, #text - 1)
			elseif not allowed[text] then	
				value = self:updateSlider(slider, nil, tostring(text) or value, min, max)
				callback(value)
			end
		end)
